...
export function PlanSection() {
  ...
  const visible = itineraries.filter(
    (it) =>
      ...
      <Reveal className="mt-12 space-y-7">
        <fieldset>
          ...
          <div className="mt-4 flex flex-wrap gap-2.5">
            <button type="button" onClick={() => setDuration("all")} className={chip(duration === "all")}>
              {t(ui.plan.all)}
            </button>
            {durations.map((d) => (
              <button
        ...
        <fieldset>
          ...
          <div className="mt-4 flex flex-wrap gap-2.5">
            <button type="button" onClick={() => setInterest("all")} className={chip(interest === "all")}>
              {t(ui.plan.all)}
            </button>
            {interests.map((it) => (
              <button
          ...
          {visible.map((it, i) => (
            <Reveal as="li" key={it.id} delay={(i % 3) * 80} className="flex">
              <article className="flex w-full flex-col border border-forest/20 bg-background p-7 transition-colors hover:border-forest/45">
                <p className="eyebrow">
                  {t(durations.find((d) => d.id === it.duration)!.label)}
                </p>
                <h3 className="mt-4 text-2xl leading-tight text-forest-deep">{t(it.title)}</h3>
                <p className="mt-3 text-sm leading-relaxed text-muted-foreground">{t(it.summary)}</p>
                ...
                <ul className="mt-6 flex flex-wrap gap-2 border-t border-forest/15 pt-4">
                  {it.interests.map((id) => (
                    <li key={id} className="text-[0.6875rem] tracking-[0.16em] uppercase text-forest/80">
