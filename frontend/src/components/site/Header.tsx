...
export function Header() {
  ...
  // Scroll lock while the mobile menu is open. `body { overflow: hidden }`
  // alone is unreliable on iOS Safari / in-app browsers — with a very tall
  // body it clips `position: fixed` children partway down the screen. Pinning
  // the body with `position: fixed` collapses it to the viewport so the
  // overlay covers the full height everywhere.
  useEffect(() => {
    if (!open) return;
  ...
  return (
    <>
      ...
      {/* Mobile menu — portalled straight to <body> so no ancestor's
          transform / filter / backdrop-filter can ever become its
          containing block and clip the full-screen overlay. */}
      {mounted &&
        createPortal(
          <div
            className={`fixed inset-0 z-50 h-dvh xl:hidden ${
